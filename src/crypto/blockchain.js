...
exports.getLastBlock = () => blockchain.sequence[blockchain.sequence.length - 1];
...
const adjustComplexity = (chain) => {
    const lastBlock = exports.getLastBlock();
    if (lastBlock.index % constants_1.COMPLEXITY_ADJUST_INTERVAL === 0 && lastBlock.index !== 0) {
        const prevBlock = chain.sequence[chain.sequence.length - constants_1.COMPLEXITY_ADJUST_INTERVAL];
...
exports.createNewBlock = (content) => {
    const prevBlock = exports.getLastBlock();
    let newBlock = {
...
exports.uploadBlock = (block) => {
    if (!validateBlock(block, exports.getLastBlock()))
        return false;
...
exports.createNewLastBlock = () => {
    const miningTx = transaction_1.createMiningTx(wallet_1.getPublicKey(), exports.getLastBlock().index + 1);
