...
exports.createNewBlockForTx = (endPoint, amount) => {
    if (!utility_1.validateAddress(endPoint))
        return null;
    const miningTx = exports.createMiningTx(wallet_1.getPublicKey(), blockchain_1.getLastBlock().index + 1);
    const tx = wallet_1.initTransaction(wallet_1.retrievePrivateKey(), pooling_1.getPool().transactions, blockchain_1.getAvailableTxOuts(), amount, endPoint);
