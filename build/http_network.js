...
exports.runHttpServer = () => {
    ...
    app.use((error, request, response, next) => { if (error)
        response.status(404).send(error); });
    app.get('/blockchain', (request, response) => response.send(blockchain_1.getBlockchain()));
    app.get('/block/:hash', (request, response) => response.send(_.find(blockchain_1.getBlockchain(), (block) => block.hash === request.params.hash)));
    ...
    app.get('/address/:address', (request, response) => response.send({
    ...
    app.post('/mine-next', (request, response) => {
        const newBlock = blockchain_1.createNextBlock();
        if (newBlock === null)
