        ...
        switch (request.type) {
            case 'LAST_BLOCK':
                connection.send(JSON.stringify({
                    type: 'RESPONSE_BLOCKCHAIN', payload: JSON.stringify([blockchain_1.getLastBlock(blockchain_1.getBlockchain())])
                }));
                return;
            case 'BLOCKCHAIN':
                connection.send(JSON.stringify({
...
const sendResponse = (newBlocks) => {
    const lastBlock = blockchain_1.getLastBlock(blockchain_1.getBlockchain());
...
exports.broadcastLastBlock = () => {
    peers.connections.forEach((connection) => connection.send({ type: 'RESPONSE_BLOCKCHAIN', payload: JSON.stringify([blockchain_1.getLastBlock(blockchain_1.getBlockchain())]) }));
};
exports.broadcastPool = () => {
